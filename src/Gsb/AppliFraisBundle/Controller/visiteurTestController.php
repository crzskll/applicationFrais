...
class VisiteurController extends Controller
{
    ...
    public function editLigneAction($id)
    {
        $em = $this->getDoctrine()->getManager();

        $horsForfaitLigne = $em->getRepository('GsbAppliFraisBundle:HorsForfaitLigne')->find($id);

        if (!$horsForfaitLigne) {
            throw $this->createNotFoundException('Unable to find HorsForfaitLigne entity.');
        }

        $editForm = $this->createEditForm($horsForfaitLigne);
        $deleteForm = $this->createDeleteForm($id);

        return $this->render('GsbAppliFraisBundle:HorsForfaitLigne:edit.html.twig', array(
            'horsForfaitLigne'      => $horsForfaitLigne,
